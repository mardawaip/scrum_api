...
//$sql = 'SELECT *, lng AS x, lat AS y  FROM skp where kategori = "Sehati"';


$sql = "SELECT skp.skp_id, skp.tahun_pendataan, skp.kategori, skp.nama_lokasi, k.kelurahan_id, k.kecamatan_id, k.kelurahan_nama, c.kecamatan_nama, skp.foto, skp.lat, skp.lng, skp.icon_map, skp.`marker-color`, skp.`marker-symbol`, skp.`marker-size`,CONCAT('<b>LOKASI SEHATI</b><br><img src=$path/konservasi/sehati/',skp.foto,' width=250',' <br><br>',skp.nama_lokasi,'<br>',skp.tahun_pendataan,'<br>',c.kecamatan_nama,'<br>',k.kelurahan_nama) As description, skp.lng AS x, skp.lat AS y FROM skp skp, kelurahan k, kecamatan c where skp.kategori = 'Sehati' AND skp.kelurahan_id = k.kelurahan_id AND k.kecamatan_id = c.kecamatan_id AND c.kecamatan_id = 3";
